...
export const EXTRA_LEVEL_SKINS = {
    ...
    // Губка: жовта, з дірочками, великими очима й двома передніми зубами; кліпає
    sponge_cube: function (ctx, s, time) {
        r(ctx, s, "#ffe14d", -0.5, -0.5, 1, 1);
        r(ctx, s, "#fff08a", -0.5, -0.5, 1, 0.08);
        // Дірочки губки
        disc(ctx, s, "#c8b020", -0.36, -0.36, 0.06);
        disc(ctx, s, "#c8b020", 0.38, -0.34, 0.05);
        disc(ctx, s, "#c8b020", -0.4, 0.3, 0.05);
        disc(ctx, s, "#c8b020", 0.36, 0.32, 0.07);
        disc(ctx, s, "#c8b020", 0.02, -0.42, 0.04);
        // Великі очі з блакитними зіницями
        const blink = (time % 3000) > 2860;
        if (blink) {
            r(ctx, s, "#3a2a10", -0.3, -0.12, 0.22, 0.04);
            r(ctx, s, "#3a2a10", 0.08, -0.12, 0.22, 0.04);
        } else {
            disc(ctx, s, "#ffffff", -0.19, -0.13, 0.14);
            disc(ctx, s, "#ffffff", 0.19, -0.13, 0.14);
            disc(ctx, s, "#39a0ff", -0.16, -0.12, 0.06);
            disc(ctx, s, "#39a0ff", 0.16, -0.12, 0.06);
            disc(ctx, s, "#101010", -0.16, -0.12, 0.03);
            disc(ctx, s, "#101010", 0.16, -0.12, 0.03);
        }
        // Щоки й усмішка з двома зубами
        disc(ctx, s, "#ff9a7a", -0.34, 0.1, 0.06);
        disc(ctx, s, "#ff9a7a", 0.34, 0.1, 0.06);
        r(ctx, s, "#8a2a1a", -0.24, 0.12, 0.48, 0.08);
        r(ctx, s, "#ffffff", -0.08, 0.2, 0.07, 0.08);
        r(ctx, s, "#ffffff", 0.01, 0.2, 0.07, 0.08);
        // Білий комірець, краватка й коричневі штанці знизу
        r(ctx, s, "#ffffff", -0.5, 0.34, 1, 0.06);
        r(ctx, s, "#8a5a2a", -0.5, 0.4, 1, 0.1);
        tri(ctx, s, "#e0203a", -0.05, 0.34, 0.05, 0.34, 0, 0.46);
        frame(ctx, s, "#c8a020");
    },

    // Кубик-ніндзя з «цеглинок»: червоний костюм, маска з прорізом для очей, кнопки зверху
    ninja_cube: function (ctx, s, time) {
        // Шипи-кнопки конструктора на верхній грані
        r(ctx, s, "#c8202a", -0.36, -0.6, 0.2, 0.1);
        r(ctx, s, "#c8202a", 0.16, -0.6, 0.2, 0.1);
        r(ctx, s, "#e0303a", -0.5, -0.5, 1, 1);
        r(ctx, s, "#ff5a5a", -0.5, -0.5, 1, 0.08);
        // Проріз маски з жовтим обличчям
        r(ctx, s, "#ffd84a", -0.38, -0.2, 0.76, 0.26);
        const look = Math.sin(time * 0.002) * 0.04;
        r(ctx, s, "#101010", -0.24 + look, -0.12, 0.1, 0.1);
        r(ctx, s, "#101010", 0.14 + look, -0.12, 0.1, 0.1);
        r(ctx, s, "#101010", -0.28, -0.18, 0.18, 0.03);
        r(ctx, s, "#101010", 0.1, -0.18, 0.18, 0.03);
        // Золота емблема й пояс
        r(ctx, s, "#8a1018", -0.5, 0.22, 1, 0.08);
        disc(ctx, s, "#ffcc33", 0, 0.26, 0.07);
        // Кінчики пов'язки маяють ззаду
        const wave = Math.sin(time * 0.012) * 0.05;
        r(ctx, s, "#e0303a", 0.5, -0.28 + wave, 0.18, 0.06);
        r(ctx, s, "#e0303a", 0.5, -0.18 - wave, 0.14, 0.06);
        frame(ctx, s, "#ffcc33");
    },

    // Кіборг: сріблястий металевий череп-робот із червоним оком, що сканує
    endo_skull: function (ctx, s, time) {
        r(ctx, s, "#8a909c", -0.5, -0.5, 1, 1);
        r(ctx, s, "#c8ced8", -0.5, -0.5, 1, 0.1);
        r(ctx, s, "#6a707c", -0.5, 0.2, 1, 0.3);
        // Заклепки й шви
        r(ctx, s, "#5a606c", -0.5, -0.24, 1, 0.03);
        disc(ctx, s, "#dce2ea", -0.4, -0.4, 0.03);
        disc(ctx, s, "#dce2ea", 0.4, -0.4, 0.03);
        // Очниці: ліве око червоне й пульсує
        r(ctx, s, "#1a1c22", -0.36, -0.16, 0.26, 0.18);
        r(ctx, s, "#1a1c22", 0.1, -0.16, 0.26, 0.18);
        const pulse = 0.6 + 0.4 * Math.sin(time * 0.008);
        disc(ctx, s, "rgba(255, 40, 40, " + pulse.toFixed(2) + ")", -0.23, -0.07, 0.07);
        disc(ctx, s, "#ff9a9a", -0.25, -0.09, 0.02);
        disc(ctx, s, "#5a2020", 0.23, -0.07, 0.05);
        // Металеві зуби
        r(ctx, s, "#1a1c22", -0.3, 0.26, 0.6, 0.14);
        for (let i = 0; i < 6; i++) {
            r(ctx, s, "#dce2ea", -0.28 + i * 0.1, 0.27, 0.07, 0.12);
        }
        // Смуга сканування
        const scan = ((time % 1800) / 1800) * 1.0 - 0.5;
        r(ctx, s, "rgba(255, 60, 60, 0.25)", -0.5, scan, 1, 0.04);
        frame(ctx, s, "#ff3a3a");
    }
};
