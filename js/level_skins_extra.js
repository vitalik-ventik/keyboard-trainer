...
export const EXTRA_LEVEL_SKINS = {
    ...
    // Кайдзю: зелена луската, спинні пластини світяться блакитним, як перед атомним променем
    kaiju_cube: function (ctx, s, time) {
        const glow = 0.5 + 0.5 * Math.sin(time * 0.006);
        const plate = "rgba(" + Math.round(90 + 120 * glow) + ", " + Math.round(190 + 60 * glow) + ", 255, 1)";
        tri(ctx, s, plate, -0.4, -0.5, -0.2, -0.5, -0.3, -0.72);
        tri(ctx, s, plate, -0.12, -0.5, 0.12, -0.5, 0, -0.8);
        tri(ctx, s, plate, 0.2, -0.5, 0.4, -0.5, 0.3, -0.72);
        r(ctx, s, "#3a5a3a", -0.5, -0.5, 1, 1);
        // Луска
        for (let y = 0; y < 4; y++) {
            for (let x = 0; x < 4; x++) {
                r(ctx, s, (x + y) % 2 === 0 ? "#2a4a2a" : "#4a6a44", -0.44 + x * 0.24, -0.44 + y * 0.24, 0.12, 0.08);
            }
        }
        // Суворі жовті очі й зубаста паща
        r(ctx, s, "#1a2a1a", -0.36, -0.2, 0.3, 0.06);
        r(ctx, s, "#1a2a1a", 0.06, -0.2, 0.3, 0.06);
        r(ctx, s, "#ffcc33", -0.3, -0.12, 0.14, 0.1);
        r(ctx, s, "#ffcc33", 0.16, -0.12, 0.14, 0.1);
        r(ctx, s, "#1a1010", -0.3, 0.14, 0.6, 0.16);
        for (let i = 0; i < 5; i++) {
            tri(ctx, s, "#f0f0e0", -0.28 + i * 0.12, 0.14, -0.2 + i * 0.12, 0.14, -0.24 + i * 0.12, 0.22);
        }
        frame(ctx, s, plate);
    },

    // Рація з 80-х: чорний корпус, антена, решітка динаміка й лампочка, що блимає під час «зв'язку»
    walkie_cube: function (ctx, s, time) {
        r(ctx, s, "#2a2a30", 0.22, -0.78, 0.08, 0.3);
        disc(ctx, s, "#3a3a44", 0.26, -0.8, 0.05);
        r(ctx, s, "#2a2a30", -0.5, -0.5, 1, 1);
        r(ctx, s, "#3a3a44", -0.5, -0.5, 1, 0.08);
        // Решітка динаміка
        for (let i = 0; i < 5; i++) {
            r(ctx, s, "#101014", -0.34, -0.34 + i * 0.1, 0.68, 0.04);
        }
        // Екранчик із частотою й кнопка
        r(ctx, s, "#1a3a1a", -0.34, 0.14, 0.4, 0.16);
        const talk = (time % 2400) < 1200;
        r(ctx, s, talk ? "#7aff5a" : "#3a8a2a", -0.3, 0.18, 0.32, 0.08);
        disc(ctx, s, talk ? "#ff3a3a" : "#5a1a1a", 0.28, 0.22, 0.07);
        r(ctx, s, "#ffcc33", -0.44, 0.38, 0.88, 0.06);
        frame(ctx, s, "#ff3a3a");
    },

    // Протектор ніндзя: помаранчевий кубик із пов'язкою, металева пластина з листком, стрічки маяють
    headband_cube: function (ctx, s, time) {
        r(ctx, s, "#ff8a1a", -0.5, -0.5, 1, 1);
        r(ctx, s, "#ffaa4a", -0.5, -0.5, 1, 0.08);
        // Жовте волосся-шипи
        tri(ctx, s, "#ffd84a", -0.5, -0.5, -0.2, -0.5, -0.4, -0.72);
        tri(ctx, s, "#ffd84a", -0.2, -0.5, 0.1, -0.5, -0.05, -0.76);
        tri(ctx, s, "#ffd84a", 0.1, -0.5, 0.4, -0.5, 0.28, -0.72);
        // Пов'язка з металевою пластиною
        r(ctx, s, "#1a2a5a", -0.5, -0.38, 1, 0.18);
        r(ctx, s, "#c8ced8", -0.24, -0.4, 0.48, 0.22);
        r(ctx, s, "#8a909c", -0.24, -0.4, 0.48, 0.03);
        // Символ-листок (спіраль із хвостиком)
        ctx.strokeStyle = "#4a505c";
        ctx.lineWidth = Math.max(1, s * 0.03);
        ctx.beginPath();
        ctx.arc(0, -0.29 * s, 0.06 * s, 0, Math.PI * 1.6);
        ctx.stroke();
        r(ctx, s, "#4a505c", 0.04, -0.34, 0.1, 0.03);
        // Стрічки ззаду
        const wave = Math.sin(time * 0.012) * 0.05;
        r(ctx, s, "#1a2a5a", 0.5, -0.36 + wave, 0.2, 0.06);
        r(ctx, s, "#1a2a5a", 0.5, -0.26 - wave, 0.16, 0.06);
        // Сині очі й вуса-смужки на щоках
        r(ctx, s, "#ffffff", -0.3, -0.1, 0.18, 0.12);
        r(ctx, s, "#ffffff", 0.12, -0.1, 0.18, 0.12);
        r(ctx, s, "#2a6aff", -0.24, -0.08, 0.08, 0.09);
        r(ctx, s, "#2a6aff", 0.18, -0.08, 0.08, 0.09);
        for (let i = 0; i < 3; i++) {
            r(ctx, s, "#6a3a10", -0.46, 0.08 + i * 0.06, 0.14, 0.02);
            r(ctx, s, "#6a3a10", 0.32, 0.08 + i * 0.06, 0.14, 0.02);
        }
        r(ctx, s, "#6a2a10", -0.14, 0.24, 0.28, 0.06);
        frame(ctx, s, "#1a2a5a");
    }
};
